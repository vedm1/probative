...
| **PB1** | Ingest: unstructured | ✅ Done | `src/probative/core/evidence.py` (mypy strict): `Tier`/`EvidenceKind`/`SourceFormat` (`StrEnum`), `Locator`, `LocatorRegion`, `Source`, `EvidenceSpan`, 8-member `IngestError` hierarchy. New `src/probative/ingest/` package: `ingest()`/`spans()`/`reextract()`, one extractor module per format (`text`, `markdown`, `csv_`, `xlsx`, `pdf`, `docx_`), each versioned (`"<format>/1"`). `reextract(source, span)` takes both args (deviates from the phase spec's original single-arg sketch — avoids denormalising `path`/`format`/`extractor_version` onto every `EvidenceSpan`). PDF via `pdfplumber` (MIT; PyMuPDF/AGPL deliberately avoided), DOCX via `python-docx`, XLSX via `openpyxl` in non-read-only mode (dodges its read-only dimension-truncation bug). Markdown heading tracking is a hand-rolled fence-aware line scanner, no new dependency. 12-file synthetic fixture corpus under `tests/fixtures/ingest/` (regenerated by committed `generate.py`; dev-only `fpdf2`/`pypdf` for PDF fixture generation) plus 7 golden normalised-text files. `.gitattributes` added (`tests/fixtures/ingest/** -text`) after discovering `git add` was silently rewriting the CRLF fixture's line endings via `core.autocrlf=input` — verified with `git show :path \| xxd` before and after. Tests: 56 in default suite (41 in `tests/ingest/`), 98–100% line coverage on every new module. `uv run ruff check`/`ruff format --check`/`mypy src`/`pytest` all green with no LLM credentials present; pre-commit clean on the full changeset. Known, documented gaps: DOCX tables and CSV fields with embedded newlines are out of scope; a PDF with any page lacking a text layer rejects the whole document, not just that page. |
| **PB2** | Ingest: structured exports (Jira + ADO) | ✅ Done | Scoped to Jira CSV/HTML/XML and ADO CSV only — Confluence split to **PB2-p2** (no real export sample existed; see OI5). `probative/core/evidence.py`: `SourceFormat` +4 (`JIRA_CSV`/`JIRA_HTML`/`JIRA_XML`/`ADO_CSV`), `Locator` +2 (`issue_key`, `field`), new `UnrecognisedTrackerFormatError`. New `probative/ingest/tracker.py` — separate public entrypoint (`ingest_tracker`/`issues`/`reextract_tracker`/`Issue`) from PB1's `ingest`/`spans`/`reextract`, because a tracker export's format can't be sniffed from a `.csv`/`.html`/`.xml` extension the way PB1's document formats can; `issues()` is a derived view over `Source.locator_regions`, built through PB1's existing `spans()` — no second source of record. New `_tracker_text.py` (shared per-issue text/region assembly), `_wiki.py` (Jira wiki-markup → text) and `_html.py` (HTML → text) normalisers — needed because real exports use *two different* markup dialects for the same logical field (Jira CSV/HTML description is wiki markup; Jira XML and ADO CSV description is real HTML), confirmed against real Jira/ADO exports provided during the session (never committed — CLAUDE.md § Secrets). Four format extractors: `jira_csv.py`/`ado_csv.py` (stdlib `csv.reader`, not `DictReader` — real exports repeat column names for multi-value fields, and `DictReader` silently keeps only the last one), `jira_html.py` (new dep **beautifulsoup4** — real status/key cells nest markup regex can't safely parse), `jira_xml.py` (stdlib `ElementTree`). Real-export findings that reshaped the spec: no dedicated Acceptance Criteria column exists in a real Jira CSV export (recognised only if present, never fabricated); Jira hierarchy is `Parent key`/`<parent id="...">`, not `Epic Link`; ADO's `Parent` is a numeric ID that commonly points **outside** the exported set (confirmed 0/17 in a real sample) — recorded verbatim, resolution deferred to the graph phases (new OI, see PROBATIVE_BUILD_PLAN.md). 12-file synthetic fixture corpus under `tests/fixtures/tracker/` (regenerated by committed `generate.py`), mirroring real structure without ever containing real content. Tests: 56 in `tests/ingest/tracker/` (112 total in the suite), 97% coverage on the new/changed modules (`tracker.py` 100%; two documented gaps mirror PB1's own: an unreachable "empty CSV after parsing" branch already caught earlier by `EmptySourceError`, and a couple of HTML/XML defensive branches no real export has been seen to hit). `uv run ruff check`/`ruff format --check`/`mypy src`/`pytest` all green with no LLM credentials present. |
| **PB2-p2** | Ingest: structured exports (Confluence) | ✅ Done | **Objective reshaped by two real Confluence Cloud per-page exports (a "Word" export and a "PDF" export) provided this session** — CLAUDE.md "reality wins over the spec": neither is a tracker/issue export. A Confluence page has no issue key, status or hierarchy; it's a title plus prose and HTML tables. The originally-planned "issues, acceptance criteria and hierarchy" objective (mirroring PB2's Jira/ADO model) is dropped — no hierarchy is invented (the PDF's nav bar lists sibling *spaces* in the account, not this page's ancestry, and treating it as structure would fabricate provenance I1 doesn't license). `probative/core/evidence.py`: `SourceFormat` +1 (`CONFLUENCE_WORD`) — no other schema changes; `Locator`'s existing `heading_path`/`line` cover this format fully. **The real "Word" export is not OOXML** — despite the `.doc` extension it is MHTML (a `multipart/related` MIME message, quoted-printable, wrapping one `text/html` part); `python-docx` would reject it. New `probative/ingest/confluence_word.py` decodes it via the stdlib `email` module (no new dependency) and BeautifulSoup (reused from PB2's `jira_html.py`), producing a text projection where each table row is flattened to one line (`"Col1 \| Col2 \| Col3"`) and a multi-`<p>` cell's paragraphs are joined with `" / "` — a normalisation this codebase hadn't needed before (`docx_.py` punts on tables entirely; PB2's generic `_html.py` doesn't separate `<td>`/`<th>` cells at all, having been built for Jira/ADO description fields, not full pages). New `probative/ingest/confluence.py` (`ingest_confluence`/`reextract_confluence`) mirrors PB2's `tracker.py` precedent exactly: format is caller-declared, not sniffed from `.doc`, because that extension is ambiguous with genuine legacy binary Word (which this codebase doesn't support). No `issues()`-equivalent exists — callers use PB1's existing `probative.ingest.spans()` directly against the plain `Source` this produces. **The real "PDF" export needed zero new code** — confirmed via `pdfplumber` (PB1's own `pdf.py`, unmodified) that it's a well-formed PDF; the one real finding is that the export is a print of the whole browser page, so extracted text carries the top navigation chrome verbatim ahead of the actual content (not a bug — it's genuinely on the page) — locked in with a characterisation test (`test_pdf_export.py`) that calls PB1's `ingest()` unchanged, no source change. 7-file synthetic fixture corpus under `tests/fixtures/confluence/` (regenerated by committed `generate.py`; reuses the `fpdf2` dev dependency from PB1's own PDF fixtures), never containing the real files' content (CLAUDE.md § Secrets). Tests: 19 in `tests/ingest/confluence/` (131 total in the suite), 100% line+branch coverage on both new modules. `uv run ruff check`/`ruff format --check`/`mypy src`/`pytest` all green with no LLM credentials present. OI5 narrows rather than closes: the per-page Word/PDF variants are now confirmed; Confluence's space-level bulk XML/HTML export remains unconfirmed and unbuilt. |
...
**PB2** — A Jira CSV export (in either its "visible fields" or "all fields" column-set shape), a Jira HTML export, a Jira XML export and an ADO CSV export each produce sources with recognised issue keys, hierarchy (`parent_key`) and acceptance criteria where present. Column-order variation and repeated column names (real Jira "all fields" exports repeat several) do not break parsing; unrecognised columns are simply not extracted. Acceptance criteria is populated only when the export actually has that field — never fabricated. **PB2-p2** carries the same bar for Confluence once a real export sample exists.
