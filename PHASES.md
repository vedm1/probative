...
| **PB2-p2** | Ingest: structured exports (Confluence) | 🔲 Planned | Confluence export → sources plus recognised issues, acceptance criteria and hierarchy. Split from PB2 because no real Confluence export sample was available this session — CLAUDE.md "reality wins over the spec" / "don't build on an unconfirmed assumption": guessing at HTML/XML/PDF export variants was rejected in favour of waiting for a real sample. **Do not start without one.** |
